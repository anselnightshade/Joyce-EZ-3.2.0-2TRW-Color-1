...
double get_average_motor_position(const std::vector<pros::Motor>& motors) {
    std::vector<double> valid_positions;

    // Collect valid motor positions
    for (const auto& motor : motors) {
        double position = motor.get_position();
        if (std::isfinite(position)) { // Ensure the position is finite
            valid_positions.push_back(position);
        }
    }

    // If no valid positions exist, return 0.0
    if (valid_positions.empty()) {
        return 0.0;
    }

    // Sort valid positions from high to low
    std::sort(valid_positions.begin(), valid_positions.end(), std::greater<>());

    // If only one valid position exists, return it
    if (valid_positions.size() == 1) {
        return valid_positions[0];
    }

    // Otherwise, calculate the average of the top two values
    return (valid_positions[0] + valid_positions[1]) / 2.0;
}

double get_average_motor_velocity(const std::vector<pros::Motor>& motors) {
    std::vector<double> valid_velocities;

    // Collect valid motor velocities
    for (const auto& motor : motors) {
        double velocity = motor.get_actual_velocity();
        if (std::isfinite(velocity)) { // Ensure the velocity is finite
            valid_velocities.push_back(velocity);
        }
    }

    // If no valid velocities exist, return 0.0
    if (valid_velocities.empty()) {
        return 0.0;
    }

    // Sort valid velocities from high to low
    std::sort(valid_velocities.begin(), valid_velocities.end(), std::greater<>());

    // If only one valid velocity exists, return it
    if (valid_velocities.size() == 1) {
        return valid_velocities[0];
    }

    // Otherwise, calculate the average of the top two values
    return (valid_velocities[0] + valid_velocities[1]) / 2.0;
}
...
int Drive::drive_sensor_right_raw() {
  ...
  return static_cast<int>(get_average_motor_position(right_motors));
}
...
// Helper function to calculate the average of the top 2 valid values
template <typename Func>
double calculate_top_2_average(const std::vector<pros::Motor>& motors, Func getter) {
    std::vector<double> valid_values;

    // Collect valid motor values using the getter function
    for (const auto& motor : motors) {
        double value = getter(motor);
        if (std::isfinite(value)) { // Ensure the value is finite (not NaN or inf)
            valid_values.push_back(value);
        }
    }

    // If no valid values exist, return 0.0
    if (valid_values.empty()) {
        return 0.0;
    }

    // Sort valid values from high to low
    std::sort(valid_values.begin(), valid_values.end(), std::greater<>());
...
int Drive::drive_velocity_right() {
    // Use the helper function to calculate the top-2 average velocity
    return static_cast<int>(calculate_top_2_average(right_motors, [](const pros::Motor& motor) {
        return motor.get_actual_velocity();
    }));
}

double Drive::drive_mA_right() {
    // Use the helper function to calculate the top-2 average current draw
    return calculate_top_2_average(right_motors, [](const pros::Motor& motor) {
        return motor.get_current_draw();
    });
}

bool Drive::drive_current_right_over() {
    // Check if any motor in the right group is over current
    for (const auto& motor : right_motors) {
        if (motor.is_over_current()) {
            return true; // If any motor is over current, return true
        }
    }

    return false; // If none are over current, return false
}
...
int Drive::drive_sensor_left_raw() {
  ...
  return static_cast<int>(get_average_motor_position(left_motors));
}
...
int Drive::drive_velocity_left() {
    // Use the helper function to calculate the top-2 average velocity
    return static_cast<int>(calculate_top_2_average(left_motors, [](const pros::Motor& motor) {
        return motor.get_actual_velocity();
    }));
}

double Drive::drive_mA_left() {
    // Use the helper function to calculate the top-2 average current draw
    return calculate_top_2_average(left_motors, [](const pros::Motor& motor) {
        return motor.get_current_draw();
    });
}

bool Drive::drive_current_left_over() {
    // Check if any motor in the left group is over current, excluding invalid (inf) values
    for (const auto& motor : left_motors) {
        if (motor.is_over_current()) {
            return true; // If any motor is over current, return true
        }
    }
